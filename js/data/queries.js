...
const ENTITY_NETWORK = MOCK_ENTITIES;

function entityRank(a, b) {
  const sa = getHairQooScore(a, ENTITY_NETWORK);
  const sb = getHairQooScore(b, ENTITY_NETWORK);
...
export function getTrending(limit = 8) {
  ...
  const ranked = improveRankingFromFeedback(
    MOCK_ENTITIES.map((e) => ({
      ...e,
      finalScore: (e.engagement?.views ?? 0) / 5000 + getHairQooScore(e, ENTITY_NETWORK) / 200,
    })),
    { profile }
  );
...
export function getFeedPage(cursor = null, pageSize = 5) {
  const ranked = [...MOCK_ENTITIES].sort(entityRank);
...
/** Leading award nominee per category (votes + HairQoo Score). */
export function getAwardLeader(category) {
  return getAwardNominee(category, MOCK_ENTITIES);
}

/** Wyszukiwanie globalne z rankingiem inteligentnym. */
export function search(query, filters = {}) {
  ...
  const ranked = rankSearchResults(q, matched, { lang, profile, network: ENTITY_NETWORK });
...
export function getEntitiesByOwner(ownerId) {
  return MOCK_ENTITIES.filter((e) => e.ownerId === ownerId).sort(entityRank);
}

/** Asystent AI — intent + ranking z warstwy uczenia. */
...
// Re-export intelligence API for hub consumers
